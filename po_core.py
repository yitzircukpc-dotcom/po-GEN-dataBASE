...
def report_pos_needing_followup(conn, days=None):
    ...
    Batch 175: the HAVING clause here used to reference last_touch (the
    SELECT list's own alias for COALESCE(MAX(ev.at), po.created_at))
    directly -- SQLite happily allows a HAVING clause to see a SELECT
    alias, but standard Postgres does not (HAVING is evaluated before the
    SELECT list even exists), so this raised a real, live
    'column "last_touch" does not exist' error against the hosted
    backend. ORDER BY referencing the same alias is fine in both engines
    (this part was never the bug) -- only HAVING needed to change. Fixed
    by wrapping the whole thing in a subquery: last_touch becomes a real
    column of the derived table `sub`, which an outer WHERE (not HAVING)
    can reference freely in both SQLite and Postgres, with identical
    results to before in either case."""
    if days is None:
        try:
    ...
    rows = conn.execute(
        ...
        "GROUP BY po.id"
        ") sub "
        "WHERE datetime(last_touch) <= datetime(?) "
